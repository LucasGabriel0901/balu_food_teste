...
if (btnNovo) {
  btnNovo.addEventListener("click", prepararNovoBannerPublicidade);
}
...
if (search) {
  search.addEventListener("input", renderizarPainelBanners);
}

if (filterStatus) {
  filterStatus.addEventListener("change", renderizarPainelBanners);
}

if (tipoMidia) {
  tipoMidia.addEventListener("change", atualizarCamposMidiaBanner);
}
    ...
    "<tr>" +
      ...
      "<td>" +
        "<div class='table-actions'>" +
          "<button type='button' class='btn-icon' title='Ativar/Inativar' onclick='alternarStatusBannerPublicidade(\"" + escapeAttrBanner(banner.id) + "\")'><i data-lucide='power'></i></button>" +
          "<button type='button' class='btn-icon' title='Editar' onclick='editarBannerPublicidade(\"" + escapeAttrBanner(banner.id) + "\")'><i data-lucide='edit-3'></i></button>" +
          "<button type='button' class='btn-icon danger' title='Excluir' onclick='excluirBannerPublicidade(\"" + escapeAttrBanner(banner.id) + "\")'><i data-lucide='trash-2'></i></button>" +
        "</div>" +
      "</td>" +
    "</tr>";
}).join("");
...
var ativos = bannersPainelCache.filter(function (banner) {
...
if (areaImagem) {
  areaImagem.style.display = tipo === "imagem" ? "flex" : "none";
}

if (areaVideo) {
  areaVideo.style.display = tipo === "video" ? "flex" : "none";
}
...
preview.innerHTML =
  "<div>" +
    "<strong>" + escapeHtmlBanner(titulo) + "</strong>" +
    "<small style='display:block;margin-top:4px;color:rgba(255,255,255,.8);font-size:11px;'>" + escapeHtmlBanner(subtitulo) + "</small>" +
  "</div>";
...
document.querySelectorAll(".bannerPaginaCheck").forEach(function (check) {
  if (check.checked) {
    selecionadas.push(check.value);
  }
});

if (selecionadas.indexOf("todas") >= 0) {
  return ["todas"];
}
